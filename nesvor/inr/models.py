...
class INR(nn.Module):
    def __init__(
        ...
        self.encoder = KPlanesFeatureEncoder(
            ...
            multiscale_res=[1, 2, 4], # 多尺度倍率
            v3d_scale_indices=[0, 1],     # 【魔法阵眼】：V_3D 仅使用前两个低频尺度！
            fusion_mode=getattr(args, "fusion_mode", "mul"),
        )
...
class NeSVoR(nn.Module):
    ...
    def forward(
            ...
            xyz_rigid = xyz_transformed

            if self.args.deformable:
                if self.args.n_features_deform > 0:
                    e_deform = self.deform_embedding(slice_idx)[:, None].expand(
                        -1, n_samples, -1
                    )
                else:
                    e_deform = xyz_transformed.new_zeros((batch_size, n_samples, 0))
                xyz_transformed = self.deform_net(xyz_transformed, e_deform)
            else:
                e_deform = None
...
# ==========================================
            # 【核心修复】：必须将展平的一维结果 Reshape 回 [Batch, n_samples] 
            # ==========================================
            density = results["density"].view(batch_size, n_samples)
            
            if "log_bias" in results:
                bias = results["log_bias"].view(batch_size, n_samples).exp()
            ...
            if "log_var" in results:
                var = results["log_var"].view(batch_size, n_samples).exp()
            else:
                var = 1
            ...
            # 此时 density 是 [B, S]，mean(-1) 会正确对每个 Ray 求平均，返回 [B]
            v_out = (bias * density).mean(-1) 
            v_out = c * v_out
            
            # 【核心修复】：为 v_out 增加最后一个维度，匹配真实值 v 的 [B, 1]，防止 [B, B] 广播灾难！
            v_out = v_out.unsqueeze(-1)  # 变为 [B, 1]
            ...
            if not self.args.no_pixel_variance:
                ...
                var = c.detach() * var
                var = var**2
                var = var.unsqueeze(-1)  # 【同理】：变为 [B, 1]
            if not self.args.no_slice_variance:
                var = var + self.log_var_slice.exp()[slice_idx].unsqueeze(-1) # 确保广播正确

            # 7. 计算基础 Loss 体系 (保真度与位姿)
            # 此时 v_out [B, 1] 与 v [B, 1] 精准相减
            ...
            if self.args.n_levels_bias and "log_bias" in results:
                # Penalize bias amplitude directly, not only global mean.
                losses[B_REG] = (results["log_bias"] ** 2).mean()

            if self.args.deformable and e_deform is not None:
                losses[D_REG] = self.deform_reg(xyz_transformed, xyz_rigid, e_deform)
    ...
    def kplanes_reg(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
            ...
            active_scale_ids = self.inr.encoder.get_active_scale_ids()
            for scale_id in active_scale_ids:
                grids = self.inr.encoder.grids[scale_id]
                # ==================================
