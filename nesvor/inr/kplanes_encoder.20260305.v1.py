...
class KPlanesFeatureEncoder(nn.Module):
    def __init__(
        ...
        log.info(f"K-Planes Encoder Init: V_4D dim={self.v4d_feature_dim}, V_3D dim={self.v3d_feature_dim} (Using scales {self.v3d_scale_indices})")
    ...
    def forward(self, xyzt: torch.Tensor):
        ...
        for scale_id, grid in enumerate(self.grids):
            # 2.1 提取当前尺度的 3D 空间特征
            v3d_scale = 1.
            for c1, c2, g_idx in spatial_idx:
                feature_dim = grid[g_idx].shape[1]
                interp_out = grid_sample_wrapper(grid[g_idx], pts_normalized[..., [c1, c2]]).view(-1, feature_dim)
                v3d_scale = v3d_scale * interp_out

            # 2.2 提取当前尺度的 4D 时空特征
            v4d_scale = v3d_scale
            for c1, c2, g_idx in temporal_idx:
                feature_dim = grid[g_idx].shape[1]
                interp_out = grid_sample_wrapper(grid[g_idx], pts_normalized[..., [c1, c2]]).view(-1, feature_dim)
                v4d_scale = v4d_scale * interp_out
