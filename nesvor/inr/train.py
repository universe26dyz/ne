...
def render_and_evaluate(model, val_batch, spatial_scaling, center, save_dir, iteration, val_mask_2d):
    ...
    slice_R = val_batch["slice_R"].to(model.args.device)
    ...
    # 与 train forward 对齐：使用 PSF Monte Carlo 积分
    n_samples_eval = int(getattr(model.args, "eval_n_samples", model.args.n_samples))
    batch_size = xyzt_scaled.shape[0]
    xyz = xyzt_scaled[:, :3]
    t = xyzt_scaled[:, 3:]

    xyz_psf_local = torch.randn(
        batch_size, n_samples_eval, 3, dtype=xyz.dtype, device=xyz.device
    )
    psf_sigma = model.psf_sigma.to(xyz.device)[slice_idx]
    ...
    xyz_transformed = ax_transform_points(
        t_pose, xyz[:, None] + xyz_psf_world, model.trans_first
    )

    t_expanded = t.unsqueeze(1).expand(batch_size, n_samples_eval, 1)
    xyzt_infer = torch.cat([xyz_transformed, t_expanded], dim=-1)

    if model.args.n_features_slice:
        se = model.slice_embedding(slice_idx)[:, None].expand(-1, n_samples_eval, -1)
        se_flat = se.reshape(-1, se.shape[-1])
    else:
        se = None
        se_flat = None

    results = model.net_forward(xyzt_infer.reshape(-1, 4), se_flat)

    density = results["density"].view(batch_size, n_samples_eval)
    if "log_bias" in results:
        bias = results["log_bias"].view(batch_size, n_samples_eval).exp()
    else:
        bias = torch.ones_like(density)

    c = F.softmax(model.logit_coef, 0)[slice_idx] * model.n_slices if not model.args.no_slice_scale else 1.0
    if isinstance(c, torch.Tensor):
        c = c.squeeze(-1)
    v_pred = (bias * density).mean(-1) * c
...
def render_and_save_nifti(model, bb_4d, save_dir, iteration, spatial_res=(128, 128, 64), n_frames=8):
    ...
    model.eval()
    os.makedirs(save_dir, exist_ok=True)
    device = torch.device(model.args.device)
    bb_4d = bb_4d.to(device=device, dtype=torch.float32)
    min_xyz = bb_4d[0, :3]
    max_xyz = bb_4d[1, :3]
    ...
    output_psf_samples = int(getattr(model.args, "output_psf_samples", model.args.n_samples))
    output_psf_samples = max(1, output_psf_samples)
    output_psf_sigma_scale = float(getattr(model.args, "output_psf_sigma_scale", 0.5))
    effective_chunk_size = max(1, chunk_size // output_psf_samples)

    voxel_size = (max_xyz - min_xyz) / torch.tensor(
        [
            max(spatial_res[0] - 1, 1),
            max(spatial_res[1] - 1, 1),
            max(spatial_res[2] - 1, 1),
        ],
        dtype=min_xyz.dtype,
        device=min_xyz.device,
    )
    output_sigma = voxel_size * output_psf_sigma_scale
    output_sigma = output_sigma.to(device=device, dtype=min_xyz.dtype)
    ...
    for t_val in t_steps:
        ...
        densities = []
        for i in range(0, xyzt_flat.shape[0], effective_chunk_size):
            chunk = xyzt_flat[i:i + effective_chunk_size]
            ...
            if output_psf_samples > 1:
                ...
                ) * output_sigma[None, None, :]
                xyz_jitter = xyz_chunk[:, None, :] + noise
...
# 【修复 1】：将全局 [3] 的分辨率张量，复制扩展成 [N_slices, 3]，满足底层查表要求
    ...
    model.train()
    encoder = model.inr.encoder

    c2f_enabled = getattr(args, "coarse_to_fine_enable", False)
    if c2f_enabled:
        if getattr(args, "c2f_stage_iters", None) is None:
            s1 = max(1, args.n_iter // 3)
            s2 = max(s1 + 1, (2 * args.n_iter) // 3)
        else:
            s1, s2 = [int(v) for v in args.c2f_stage_iters]
        if not (0 <= s1 < s2):
            raise ValueError(
                f"Invalid c2f_stage_iters={args.c2f_stage_iters}. Expected 0 <= s1 < s2."
            )
        logging.info(
            "Coarse-to-fine enabled: stage1<%d -> [1], %d<=iter<%d -> [1,2], iter>=%d -> [1,2,4]",
            s1,
            s1,
            s2,
            s2,
        ...
        s2 = None
        logging.info(
            "Coarse-to-fine disabled: active_scales=%s",
            encoder.multiscale_res_multipliers,
        )

    current_active_scale_mask = None
    loss_weights = {
        D_LOSS: 1.0,
        S_LOSS: 1.0,
    ...
    for i in pbar:  # <--- 使用 pbar 替代原来的 range
        ...
        if c2f_enabled:
            if i < s1:
                next_active_scale_mask = [True, False, False]
                current_stage = "stage1"
            elif i < s2:
                next_active_scale_mask = [True, True, False]
                current_stage = "stage2"
            else:
                next_active_scale_mask = [True, True, True]
                current_stage = "stage3"
        else:
            next_active_scale_mask = [True] * len(encoder.multiscale_res_multipliers)
            current_stage = "baseline"

        if current_active_scale_mask != next_active_scale_mask:
            encoder.set_active_scale_mask(next_active_scale_mask)
            current_active_scale_mask = list(next_active_scale_mask)
            logging.info(
                "Iter %d: %s active_scales=%s",
                i,
                current_stage,
                encoder.get_active_scale_values(),
            )
