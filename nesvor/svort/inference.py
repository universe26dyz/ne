...
def svort_predict(
    ...
    if svort:
        ...
        )  ## 加载字典中的保存的checkpoint的链接，下载cp，并加载权重
        ...
        model.to(device)
        model.eval() #告诉模型现在是测试，而不是训练
    return run_svort(dataset, model, svort, vvr, force_vvr, force_scanner) # 无论svort是否为True，都要运行run_svort
