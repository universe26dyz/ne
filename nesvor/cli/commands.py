...
class Reconstruct(Command):
    ...
    def preprocess(self) -> Dict[str, Any]:
        ...
        if "input_stacks" in input_dict and input_dict["input_stacks"]:
            ...
            input_dict["input_slices"] = _register(
                self.args, input_dict["input_stacks"]
            ) #体积粗对齐（vvr-传统stack配准），并且将配准后的slice保存到input_dict["input_slices"]中
    ...
    def exec(self) -> None: # 作用：执行重建
        ...
        output_volume, simulated_slices = _sample_inr( #从训练好的模型进行推理阶段，即根据模型预测结果，生成重建的volume和simulated slices
            self.args,
            model,
